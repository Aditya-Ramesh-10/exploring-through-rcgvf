...
class PPOAlgoNovelD(PPOAlgoWithIR):
    """Uses intrinsic rewards from NovelD."""
    ...
    def __init__(self, envs, acmodel, device=None, num_frames_per_proc=None, 
        ...
        self.ir_scale_alpha = ir_scale_alpha
    ...
    def compute_intrinsic_rewards(self, final_obs, kw_dict={}):
        """
        Computes the intrinsic rewards for the given observations using NovelD
        """
        ...
        all_obs = all_obs + [final_obs[j] for j in range(self.num_procs)]
        ...
        self.rnd_targets = rnd_targets.reshape(self.num_frames_per_proc + 1, self.num_procs, 
                                               self.target_dim)

        predictions = predictions.reshape(self.num_frames_per_proc + 1, self.num_procs, 
                                               self.target_dim)

        novelty_scores = torch.norm(self.rnd_targets - predictions, dim=-1, p=2)
        novelty_diff = novelty_scores[1:] - self.ir_scale_alpha * novelty_scores[:-1]
        intrinsic_rewards = torch.clamp(novelty_diff, min=0.0)
