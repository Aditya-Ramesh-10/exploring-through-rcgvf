...
from ensemble_model import EnsembledHistoryConditionedRecurrentFeatureNetwork

from agents.ppo_rcgvf import PPOAlgoRCGVF
...
config_defaults = dict(
    ...
    record_room_data=False, # only works for multiroom environments
...
def main():
    ...
    predictor = EnsembledHistoryConditionedRecurrentFeatureNetwork(obs_space,
                                                                   output_embedding_size=args.rnd_target_embedding_size,
    ...
    if args.algo == "ppo-rcgvf":
        algo = PPOAlgoRCGVF(envs, acmodel, device, args.frames_per_proc, args.discount,
                          ...
                          record_room_data=args.record_room_data, 
                          use_episodic_counts=args.use_episodic_counts,
                          pseudo_reward_generator=random_target_network,
                          predictor=predictor, predictor_lr=args.lr * args.lr_predictor_factor,
                          gvf_discount=args.gvf_discount, gvf_lambda=args.gvf_lambda)
    ...
    while num_frames < args.frames:
        ...
        if update % args.log_interval == 0:
            ...
            if 'rcgvf' in args.algo:
                ...
                header += ["mean_cumulants_batch", "min_cumulants_batch",
                          "max_cumulants_batch", "std_cumulants_batch"]

                data += [logs["mean_cumulants_batch"], logs["min_cumulants_batch"],
                        logs["max_cumulants_batch"], logs["std_cumulants_batch"]]

                header += ["mean_pseudo_return_batch", "min_pseudo_return_batch",
                          "max_pseudo_return_batch", "std_pseudo_return_batch"]

                data += [logs["mean_pseudo_return_batch"], logs["min_pseudo_return_batch"],
                        logs["max_pseudo_return_batch"], logs["std_pseudo_return_batch"]]

                header += ["mean_prediction_error_batch", "min_prediction_error_batch",
                          "max_prediction_error_batch", "std_prediction_error_batch"]

                data += [logs["mean_prediction_error_batch"], logs["min_prediction_error_batch"],
                        logs["max_prediction_error_batch"], logs["std_prediction_error_batch"]]

                header += ["mean_variance_predictions_batch", "min_variance_predictions_batch",
                          "max_variance_predictions_batch", "std_variance_predictions_batch"]

                data += [logs["mean_variance_predictions_batch"], logs["min_variance_predictions_batch"],
                        logs["max_variance_predictions_batch"], logs["std_variance_predictions_batch"]]
